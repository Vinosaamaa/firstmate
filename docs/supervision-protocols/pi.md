...
When `config/pi-supervision-branch` explicitly grants the one project resolved from an ordinary actionable wake, the watcher extension hands only that wake's claimed eligible rows to the persistent in-process supervision branch while main-only rows remain queued for this conversation.
Every meaningful routine outcome returns as an appended, rendered note that leads with ⛵ then the dim outcome text and omits branch-mechanics boilerplate.
...
This conversation still receives every ungranted, mixed-project, fleet-wide, or unresolvable wake, the branch's wakes when it is unavailable or away mode is active, and every watcher-failure alarm regardless, so the arm and repair contract above is unchanged.
