...
**[`no-mistakes`](https://github.com/kunchenguid/no-mistakes) is an opt-in delivery path for pull requests targeting `main`.**
Use it when the captain or maintainer explicitly requests it, or when you want its isolated review, test, and lint pipeline before opening a pull request.
Standard pull requests may be pushed directly and rely on the repository's normal CI.

`no-mistakes` puts a local git proxy in front of your real remote.
Pushing through it runs an AI-driven review/test/lint pipeline in an isolated worktree, forwards the push upstream only after every check passes, and opens a clean PR automatically.

A GitHub Actions check (`No-mistakes attestation`) runs on pull requests targeting `main` and validates the no-mistakes signature and structured attestation only when the canonical no-mistakes signature is present.
...
Pull requests without the signature pass this optional policy check and remain governed by the repository's ordinary CI and review requirements.
GitHub Actions and Dependabot remain exempt so their automation keeps working.

## No-mistakes workflow
...
Tracked changes to firstmate itself - `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `.github/workflows/`, `bin/`, `.agents/skills/`, and `skills/` - ship on a feature branch and require an explicit merge approval.
Use the `no-mistakes` pipeline for those changes only when the captain or maintainer explicitly requests that opt-in path.
...
When no-mistakes is explicitly requested, crewmate validation follows the installed version's SKILL.md and live `axi` help instead of duplicating gate mechanics in firstmate docs.
