...
3. **Write within the existing boundaries.**
   ...
   - Knowledge general to every Firstmate user belongs in this repo's shared tracked material through the normal Fastlane branch, PR, and captain-merge path, with no-mistakes only when the captain explicitly requests it.
