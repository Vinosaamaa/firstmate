...
phase_send() {
  ...
  local window
  window=$(sed -n 's/^window=//p' "$HOME_DIR/state/design.meta")
  assert_grep "send-keys -t $window -l Firstmate instruction waiting:" "$LOG" "send did not ring the window recorded in this home's meta"
