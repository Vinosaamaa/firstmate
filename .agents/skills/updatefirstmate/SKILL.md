...
Firstmate is its own repo, so new tracked material (`AGENTS.md`, `bin/`, `.agents/skills/`, and public `skills/`) reaches `main` through Fastlane or an explicitly requested no-mistakes run and then sits there until each running firstmate pulls it.
