...
When the supervision branch is explicitly enabled for a wake's one queue-lock-bound project (`config/pi-supervision-branch` lists its exact `project=` metadata value; docs/pi-supervision-branch.md), the watcher extension hands that ordinary actionable batch to the persistent in-process supervision branch instead of this conversation, and branch outcomes return as appended "⎇ branch merged [...]" notes, of which only captain-relevant ones open a turn.
