...
`data/` holds durable private fleet records such as the Firstmate and crew identity records, project, external-workspace, and secondmate registries, captain preferences, optional shared captain preferences, learnings, backlog, briefs, scout reports, and explicitly installed content-addressed extension packages under `data/extensions/packages/`.
...
The branch prompt owns the unconditional explicit-request rule and the distinction between captain-facing and unsolicited routine outcomes.
